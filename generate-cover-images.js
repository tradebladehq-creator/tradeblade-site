...
// ─── TRANSLATIONS (same as generate-covers.js) ───
const T = {
  'en': {
...
// ─── HTML TEMPLATE (same layout, sized for email: 480x720) ───
function buildCoverHTML(lang, t, bgDataUri) {
  ...
  // Sizing for 480x720 (email-friendly dimensions)
  const m1Len = t.main1.length;
  const m2Len = t.main2.length;
  const maxLen = Math.max(m1Len, m2Len);
  let mainFontSize = '34px';
  if (maxLen > 14) mainFontSize = '26px';
  else if (maxLen > 11) mainFontSize = '28px';
  else if (maxLen > 9) mainFontSize = '31px';
...
async function main() {
  ...
  const bgImagePath = path.join('C:', 'Users', 'Kevin', 'Documents', 'NinjaTrader 8', 'TradeBlade', 'Text-Free-Master-Cover.png');
  ...
  console.log(`Generating ${langs.length} cover images...\n`);
  ...
  for (const lang of langs) {
    ...
    const outPath = path.join(outDir, `cover-${lang}.jpg`);
    await page.screenshot({ path: outPath, type: 'jpeg', quality: 85 });
    await page.close();

    const size = Math.round(fs.statSync(outPath).size / 1024);
    console.log(`  ✓ ${lang} → cover-${lang}.jpg (${size} KB)`);
  }

  await browser.close();
  console.log(`\n✅ Generated ${langs.length} cover images in ${outDir}`);
}
